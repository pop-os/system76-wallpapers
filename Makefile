...
MAGICK_CMD := $(shell command -v magick 2>/dev/null || command -v convert 2>/dev/null)

ifeq ($(MAGICK_CMD),)
$(error "Neither 'magick' nor 'convert' was found in PATH")
endif
...
scaled/%: backgrounds/%
	...
	$(MAGICK_CMD) "$<" -resize "3840x2160^" "build/$*"
	mv "build/$*" "$@"
